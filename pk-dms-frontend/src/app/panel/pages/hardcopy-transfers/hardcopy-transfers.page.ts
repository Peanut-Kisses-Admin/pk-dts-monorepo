...
@Component({
    ...
    template: `
        <section class="space-y-6">
            <header><h1 class="text-2xl font-bold">Hardcopy Transfers</h1><p class="text-slate-500">Transfer requests remain pending until the requester or Current Holder confirms physical receipt.</p></header>
            <article *ngIf="canCreate()" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <h2 class="mb-4 text-lg font-bold">Create transfer request</h2>
                <div class="grid gap-4 md:grid-cols-2">
                    ...
                    <input [(ngModel)]="form.document_copy_number" class="select-field" placeholder="Document / Copy Number" />
                    ...
                    <input [(ngModel)]="form.transfer_to" class="select-field" placeholder="Transfer To" />
                    ...
                    <div class="storage-summary md:col-span-2" *ngIf="form.destination_location_id">
                        <span><i class="pi pi-sitemap"></i> Storage classification is filled from the selected Location</span>
                    ...
                    <textarea [(ngModel)]="form.reason" class="select-field" rows="2" placeholder="Reason"></textarea>
                </div>
                <p class="mt-3 text-sm text-slate-500">The request goes directly to the configured Plant Manager, Document Controller Officer, or Admin for approval. After approval and physical dispatch, the requester or Current Holder confirms receipt. The selected Location determines the destination classification.</p>
                <p-button class="mt-4" label="Submit transfer request" icon="pi pi-send" (onClick)="create()" />
            </article>
            <article *ngIf="canViewOwn()" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <h2 class="mb-4 text-lg font-bold">My transfer requests</h2>
                <div *ngFor="let transfer of transfers" class="mb-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-100 p-4">
                    <div><strong>{{ transfer.document?.document_title || transfer.document_id }}</strong><div class="text-sm text-slate-500">Copy {{ transfer.document_copy_number }} · {{ transfer.transfer_to }}</div></div>
                    <div class="flex items-center gap-2"><span class="status">{{ statusLabel(transfer.status) }}</span><p-button *ngIf="transfer.status === 'Draft'" size="small" label="Submit" (onClick)="submit(transfer)" /><p-button *ngIf="transfer.status === 'Returned'" size="small" label="Resubmit" (onClick)="resubmit(transfer)" /><p-button *ngIf="['Draft','ForApproval','Returned'].includes(transfer.status)" size="small" label="Cancel" severity="danger" [outlined]="true" (onClick)="cancel(transfer)" /><p-button *ngIf="transfer.status === 'PendingRecipientAcceptance' && canAccept()" size="small" label="Confirm receipt" severity="success" (onClick)="accept(transfer)" /></div>
                </div>
                <p *ngIf="!transfers.length" class="text-slate-500">No transfer requests yet.</p>
            </article>
            <article *ngIf="canReview()" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <h2 class="mb-4 text-lg font-bold">Transfers awaiting your action</h2>
                <div *ngFor="let transfer of pendingTransfers" class="mb-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-100 p-4">
                    <div><strong>{{ transfer.document?.document_title || transfer.document_id }}</strong><div class="text-sm text-slate-500">Copy {{ transfer.document_copy_number }} · {{ transfer.transfer_to }}</div></div>
                    <div class="transfer-actions"><span class="status">{{ statusLabel(transfer.status) }}</span><input *ngIf="transfer.status === 'ForApproval'" [(ngModel)]="actionComments[transfer.transfer_request_id]" class="action-note" placeholder="Decision remarks" /><p-button *ngIf="transfer.status === 'ForApproval' && canApprove()" size="small" label="Approve" (onClick)="approve(transfer)" /><p-button *ngIf="transfer.status === 'ForApproval' && canApprove()" size="small" label="Return" severity="warn" [outlined]="true" (onClick)="returnForCorrection(transfer)" /><p-button *ngIf="transfer.status === 'ForApproval' && canApprove()" size="small" label="Reject" severity="danger" [outlined]="true" (onClick)="reject(transfer)" /><p-button *ngIf="transfer.status === 'Approved' && canDispatch()" size="small" label="Prepare for transfer" (onClick)="forTransfer(transfer)" /><p-button *ngIf="transfer.status === 'ForTransfer' && canDispatch()" size="small" label="Dispatch" (onClick)="dispatch(transfer)" /></div>
                </div>
...
export class HardcopyTransfersPage implements OnInit {
    ...
    form = { document_id: '', document_copy_number: '', current_holder: '', current_holder_user_id: '', transfer_to: '', destination_area_id: '', destination_specific_id: '', destination_asset_id: '', destination_location_id: '', destination_sequence_id: '', reason: '' };
    ...
    create() {
        if (!this.form.document_id || !this.form.document_copy_number || !this.form.transfer_to || !this.form.destination_location_id || !this.form.reason) return;
        ...
        this.service.create(payload).subscribe((transfer) => {
            this.service.submit(transfer.transfer_request_id).subscribe(() => {
                this.form = { document_id: '', document_copy_number: '', current_holder: '', current_holder_user_id: '', transfer_to: '', destination_area_id: '', destination_specific_id: '', destination_asset_id: '', destination_location_id: '', destination_sequence_id: '', reason: '' };
                this.load();
            });
        });
    }
