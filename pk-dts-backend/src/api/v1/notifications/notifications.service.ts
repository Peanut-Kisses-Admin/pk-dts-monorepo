...
export class NotificationsService {
  ...
  async list(user: AuthenticatedUser) {
    ...
    const items: FeedItem[] = [
      ...
      ...histories.map((item) => ({ event_key: `status:${item.history_id}`, title: this.statusTitle(item.new_status), message: item.document.document_title, route: isAdmin && item.new_status === DocumentStatus.PendingApproval ? '/panel/approval-review' : '/panel/my-document-requests', created_at: item.created_at, icon: this.statusIcon(item.new_status) })),
